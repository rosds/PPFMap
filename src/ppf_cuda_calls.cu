...
ppfmap::cuda::setPPFMap(const float *points, 
    ...
    for (int i = 0; i < n; i++) {
        model->points_x[i] = points[3 * i + 0]; 
        model->points_y[i] = points[3 * i + 1]; 
        model->points_z[i] = points[3 * i + 2]; 
    
        model_normals->points_x[i] = normals[3 * i + 0]; 
        model_normals->points_y[i] = normals[3 * i + 1]; 
        model_normals->points_z[i] = normals[3 * i + 2]; 
    }
